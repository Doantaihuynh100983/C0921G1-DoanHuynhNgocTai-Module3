...
from furuma.loai_dich_vu ldv join furuma.dich_vu dv on ldv.ma_loai_dich_vu = dv.ma_loai_dich_vu
	  join furuma.hop_dong hd  on dv.ma_dich_vu = hd.ma_dich_vu 
where dv.ma_dich_vu NOT IN (
			select hd.ma_dich_vu from  furuma.hop_dong
            where  MONTH(ngay_lam_hop_dong) <= 3
)
